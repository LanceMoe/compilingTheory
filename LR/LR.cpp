...
void LR::showStatusStack()
{
	for (vector<int>::iterator it = status.begin(); it != status.end(); ++it)
	{
		if (*it < 10)
			...
			printf(" <span class='underline'>%d</span> ", *it);
	}
}
...
Item LR::closure(Item I)
{
	...
	for (size_t i = 0; i < I.prods.size(); ++i)
	{ // 枚举I的产生式
		// puts("======================");
		// printf("i=%ld, size:%ld\n", i, I.prods.size());
		// I.display();
		...
		if ((pointLoc = prod.right.find('.')) != string::npos && pointLoc != prod.right.length() - 1)
		{ // 找到.，A->a.Bc,d
...
{ // 构造Action、GOTO表
	...
	{ // 逐个项目集
		...
		{ // 逐个项目
			...
			{ // 不是最后一个
				char X = prod.right[pointLoc + 1];
				if (G.Vt.find(X) != G.Vt.end() && GOTO.find(make_pair(i, X)) != GOTO.end())
				{ // 终结符
